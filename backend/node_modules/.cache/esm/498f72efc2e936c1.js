let fs,jwt,AsyncLock,InputError,AccessError,resolve;_c2e‍.x([["save",()=>save],["reset",()=>reset],["resourceLock",()=>resourceLock],["getEmailFromAuthorization",()=>getEmailFromAuthorization],["login",()=>login],["logout",()=>logout],["register",()=>register],["listingLength",()=>listingLength],["addPilot",()=>addPilot],["getTimeValue",()=>getTimeValue],["getAllListings",()=>getAllListings],["assertOwnsListing",()=>assertOwnsListing],["updateIncrementListing",()=>updateIncrementListing],["updateDecrementListing",()=>updateDecrementListing],["removeListing",()=>removeListing]]);_c2e‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_c2e‍.w("jsonwebtoken",[["default",["jwt"],function(v){jwt=v}]]);_c2e‍.w("async-lock",[["default",["AsyncLock"],function(v){AsyncLock=v}]]);_c2e‍.w("./error",[["InputError",["InputError"],function(v){InputError=v}],["AccessError",["AccessError"],function(v){AccessError=v}]]);_c2e‍.w("path",[["resolve",["resolve"],function(v){resolve=v}]]);
  ...
  new Promise((resolve, reject) => {
    lock.acquire('saveData', () => {
      try {
       ...
       const save = () => update(users, listings);
       const reset = () => {
  update({}, {}, {});
  users = {};
  listings = {};
...
***************************************************************/
  ...
  resourceLock((resolve, reject) => {
    ...
    resolve();
       ...
       const register = (email, password, name, time, currentMonth) =>
  resourceLock((resolve, reject) => {
    if (email in users) {
  ...
  resourceLock((resolve, reject) => {
    resolve(
      Object.keys(listings).length
       ...
       const addPilot = (name, email, patternNumber, monthlyTime, quaterTime, yearlyTime, thumbnail) =>
  resourceLock((resolve, reject) => {
    if (name === undefined) {
  ...
  resourceLock((resolve, reject) => {
    if (!(listingId in listings)) {
      ...
      resolve();
       ...
       const updateIncrementListing = (listingId, time, m) =>
  resourceLock((resolve, reject) => {
    ...
    resolve();
       ...
       const updateDecrementListing = (listingId, time, m) =>
  resourceLock((resolve, reject) => {
    listings[listingId].monthlyTime[m] -= parseInt(time);
    listings[listingId].quaterTime[(Math.ceil(parseInt(m) / 3)).toString()] -= parseInt(time);
    listings[listingId].yearlyTime -= parseInt(time);
